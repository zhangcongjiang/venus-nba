...
if __name__ == '__main__':
    sql = """SELECT id,player_name,href FROM public.player_draft where  id>=60 and attend=TRUE order by id;"""
    players = get_sql(sql)
    for player in players:
        ...
        awards = soup.find("div", {'id', 'div_leaderboard'})
        ...
        if all_salaries:
            ...
            for k, v in salaries.items():
                data.append((player_name, int(k.split("-")[0])+1, int(v)))
            print(data)
            if data:
                sql = f"""INSERT INTO public.player_salary ("player_name", "season","salary")
                    VALUES (%s, %s,%s)
